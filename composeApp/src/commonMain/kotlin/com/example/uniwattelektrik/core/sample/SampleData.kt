...
enum class TaskPriority(val label: String, val color: Color, val bg: Color) {
    Success ("Success",    AppTheme.Success,  AppTheme.SuccessBg),
    Warning ("Medium", AppTheme.Warning,  AppTheme.WarningBg),
    Danger("Danger",   AppTheme.Danger, AppTheme.DangerBg),
}
...
object SampleTasks {
    val all: List<SampleTask> = listOf(
        SampleTask(
            ...
            priority = TaskPriority.Danger, status = TaskStatus.InProgress, assigneeInitials = "RK",
        ),
        SampleTask(
            ...
            priority = TaskPriority.Warning, status = TaskStatus.Todo, assigneeInitials = "RK",
        ),
        SampleTask(
            ...
            priority = TaskPriority.Success, status = TaskStatus.Todo, assigneeInitials = "RK",
        ),
        SampleTask(
            ...
            priority = TaskPriority.Warning, status = TaskStatus.Todo, assigneeInitials = "AM",
        ),
        SampleTask(
        ...
        SampleTask(
            ...
            priority = TaskPriority.Danger, status = TaskStatus.Done, assigneeInitials = "SP",
        ),
    )
...
enum class NotifKind(val emoji: String, val tint: Color) {
    Task   ("📋", AppTheme.Brand),
    Alert  ("⚠️", AppTheme.Danger),
    Leave  ("🏖", AppTheme.Warning),
...
enum class LeaveStatus(val label: String, val color: Color, val bg: Color) {
    Pending ("Pending",  AppTheme.Warning,  AppTheme.WarningBg),
    Approved("Approved", AppTheme.Success,  AppTheme.SuccessBg),
    Rejected("Rejected", AppTheme.Danger, AppTheme.DangerBg),
}
...
enum class EmployeeStatus(val label: String, val color: Color, val bg: Color) {
    Active   ("Active",    AppTheme.Success,   AppTheme.SuccessBg),
    OnLeave  ("On leave",  AppTheme.Warning,   AppTheme.WarningBg),
...
object SampleEmployees {
    val all: List<SampleEmployee> = listOf(
        ...
        SampleEmployee("E-1027", "Vikram Sethi",   "Operations Lead",          "Central",     2, listOf(Color(0xFFBFD7FF), Color(0xFF1A6BF5)), EmployeeStatus.Active),
...
object SampleAdminData {
    val kpis = listOf(
        ...
        KpiPoint("Employees on duty", "23",  "+2",    true,  AppTheme.Success),
        KpiPoint("Critical alerts",   "3",   "+1",    false, AppTheme.Danger),
        KpiPoint("Cost this month",   "₹4.2L","-6 %", true,  AppTheme.Brand),
    )
